        ...
        exercicios=[
            ExercicioCreate(
                ...
                sem_peso_details=SemPesoCreate(
                    tempo_seg=1800.0, distancia_m=5000.0, meta_velocidade=2.8
                ),
            )
        ],
    )
...
def test_create_training_with_weight_exercise(
    mock_com_peso,
    ...
    result = training_service.create_training(
        mock_db, sample_treino_create, sample_user.id
    )
...
def test_create_training_with_cardio_exercise(
    mock_sem_peso,
    mock_exercicio_class,
    mock_treino_class,
    mock_convert,
    ...
    result = training_service.create_training(
        mock_db, sample_treino_cardio_create, sample_user.id
    )
